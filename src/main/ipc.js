...
const DISCORD_OAUTH_URL = 'https://discord.com/oauth2/authorize?client_id=1227719606223765687&response_type=code&redirect_uri=https%3A%2F%2Fmagicgarden.gg%2Foauth2%2Fredirect&scope=identify+guilds.members.read+guilds';
let authWin = null;
...
const register = ({ getMainWindow, emitDebug, getTray }) => {
  ipcMain.handle('auth:login', async () => {
    ...
    const ses = electronSession.fromPartition(AUTH_PARTITION, { cache: false });

    // Set required cookies before opening the window
    await Promise.all([
      ses.cookies.set({
        url: 'https://magicgarden.gg',
        name: 'mc_oauth_room_id',
        value: 'MgAFK',
      ...
      ses.cookies.set({
        url: 'https://magicgarden.gg',
        name: 'mc_oauth_redirect_uri',
        value: 'https://magicgarden.gg/oauth2/redirect',
        domain: '.magicgarden.gg',
        path: '/',
        secure: false,
        httpOnly: false,
    ...
    return new Promise((resolve) => {
      ...
      authWin.once('ready-to-show', () => { if (authWin && !authWin.isDestroyed()) authWin.show(); });
      let resolved = false;
      const onCookieChanged = (_event, cookie, _cause, removed) => {
        if (removed) return;
        if (cookie.name === 'mc_jwt' && cookie.domain && cookie.domain.includes('magicgarden.gg')) {
          if (!resolved) {
            resolved = true;
      ...
      authWin.loadURL(DISCORD_OAUTH_URL);
      authWin.on('closed', () => {
        authWin = null;
        if (!resolved) {
          resolved = true;
